...
u8 WM8978_Init(void)
{
    ...
    IIC_Init();//初始化IIC接口
    res=WM8978_Write_Reg(0,1);	//软复位WM8978
    if(res)return 1;			//发送指令失败,WM8978异常
    //以下为通用设置
    WM8978_Write_Reg(1,0X7);
    WM8978_Write_Reg(1,0X2f);	//R1,MICEN设置为1(MIC使能),BIASEN设置为1(模拟器工作),VMIDSEL[1:0]设置为:11(5K)
    WM8978_Write_Reg(2,0X1B0);	//R2,ROUT1,LOUT1输出使能(耳机可以工作),BOOSTENR,BOOSTENL使能
    WM8978_Write_Reg(3,0X6C);	//R3,LOUT2,ROUT2输出使能(喇叭工作),RMIX,LMIX使能
    WM8978_Write_Reg(6,1);		//R6,MCLK由外部提供
    WM8978_Write_Reg(7,1);		//R7,使能slow clock已使用零交叉，采用率为48KHz（bit3：1）
...
void WM8978_Output_Cfg(u8 dacen,u8 bpsen)
{
    ...
    regval = WM8978_Read_Reg(3);	//读取R47
    regval |=~(3<<2);			//左右声道输出混合使能
