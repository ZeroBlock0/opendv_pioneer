...
void LCD_WR_CMD(u16 Cmd)
{
     ...
     IOWR_ALTERA_AVALON_PIO_DATA(PIO_LCD_DATA_OUT_BASE,Cmd);      		// 往DATA端口写命令
...
void LCD_WR_DATA(u16 Data)
{
     ...
     IOWR_ALTERA_AVALON_PIO_DATA(PIO_LCD_DATA_OUT_BASE,Data);
...
u16 LCD_RD_DATA()
{
     ...
     IOWR_ALTERA_AVALON_PIO_DATA(PIO_MLCD_WR_N_BASE,1);
     read_data = IORD_ALTERA_AVALON_PIO_DATA(PIO_LCD_DATA_IN_BASE);
     IOWR_ALTERA_AVALON_PIO_DATA(PIO_MLCD_RD_N_BASE,1);

     IOWR_ALTERA_AVALON_PIO_DATA(PIO_LCD_DATA_DIR_BASE,1);          // mlcd_data_dir=1 为输出
     return read_data;
}
...
void LCD_Scan_Dir(u8 dir)
{
    ...
    u16 dirreg=0;
    u16 temp;
    if((lcddev.dir==1)||(lcddev.dir==0&&lcddev.id==0X1963))//横屏时，对1963不改变扫描方向！竖屏时1963改变方向
    {
         switch(dir) {     //方向转换
         case 0:dir=6;break;
         case 1:dir=7;break;
...
{
	IOWR_ALTERA_AVALON_PIO_DATA(PIO_LCD_INIT_DONE_BASE,0);    //拉低初始化完成引脚
	delay_ms(50);
	//TFTLCD 复位
