...
u16 data_xcoord[] = {130,225,45,165,285};                   //数据显示X坐标
u16 data_ycoord[] = {8,8,256,256,256};                      //数据显示Y坐标
s32 ver_res_arr[] = {100,200,500,1000,2000,4000};           //垂直分辨率电压值
u16 sample_rate_arr[] = {1,2,5,10,20,50,100,200,500,1000};  //抽样率
u8  zoom_nul_arr[] = {10,5,2,1,2,4};                        //缩放倍数

s16 min_voltage = -5000;                                    //AD最小电压值
s16 max_voltage = 5000;                                     //AD最大电压值
s16 max_addata = 255;                                       //AD最大值
...
void DispPara_Init(){
	...
	dispdev.ver_res_gears = 4;            //默认垂直方向分辨率档位2V  0~5

	dispdev.trig_type = 1;                //上升沿触发
	dispdev.trig_gears = 8;               //触发电平档位 0 ,从下到上1~15
	//分局触发电平档位,计算出触发电平值
	dispdev.trig_level = (s32)(dispdev.trig_gears - 8) * ver_res_arr[dispdev.ver_res_gears]/2;
}
...
void Draw_WaveBorder(){
	...
	GUI_SetColor(GUI_CYAN);            //设置GUI前景色 , 边框颜色:青色
	...
	GUI_SetColor(GUI_LIGHTDARK);       //设置GUI前景色，内框方格颜色:亮黑色
	for(i = 1;i < 8;i++){
		if(i == 4)
			continue;
...
void Draw_SetDisp(){
	...
	GUI_SetPenSize(1);                  //设置画笔大小
	//绘制边框线
	GUI_SetColor(GUI_DARKCYAN);         //设置区域暗青色背景
	...
	GUI_FillRect(dispdev.wave_xstart+dispdev.wave_width,0,lcddev.width-1,lcddev.height-1);
  	GUI_SetColor(GUI_WHITE);            //设置区域白色边框
	...
	//绘制设置区域显示字符
	GUI_SetColor(GUI_YELLOW);        //设置GUI前景色,黄色字符显示
...
void Draw_DataCharDisp(){
	...
	//正点原子
	GUI_SetTextMode(GUI_TM_TRANS);
	GUI_SetColor(GUI_LIGHTGREEN);        //设置GUI前景色,亮绿色
	GUI_SetTextAlign(GUI_TA_LEFT | GUI_TA_TOP);
	GUI_DispStringAt("正点原子",11,8);
	//水平分辨率(时间)
	GUI_SetColor(GUI_LIGHTGRAY);         //设置GUI前景色,亮灰色
	...
	//绘制界面下方显示区域
	GUI_SetColor(GUI_YELLOW);             //设置GUI前景色，黄色
...
void GetVpp(){
	...
	dispdev.vpp = ad_vpp * (u32)(max_voltage-min_voltage)/(u32)(max_addata);
	if(ad_min_v >= max_addata/2)
		dispdev.min_voltage = (ad_min_v - (max_addata/2)) * (u32)(max_voltage-min_voltage)/(u32)(max_addata);
	else{
		dispdev.min_voltage = ((max_addata/2) - ad_min_v) * (u32)(max_voltage-min_voltage)/(u32)(max_addata);
...
void SendTriglevel(){
	...
	if(dispdev.trig_level < 0)
		s_trig_addata = max_addata/2-(dispdev.trig_level*max_addata/2/min_voltage);
	else
		s_trig_addata = max_addata/2+dispdev.trig_level*max_addata/2/max_voltage;
	if(s_trig_addata<0)
		s_trig_addata = 0;
	...
	trig_pixel = TrigLineMeas();
	IOWR(AVALON_MM_BRIDGE_1_BASE,3, (trig_pixel<<8) | trig_addata);
}
...
alt_u8 TrigLineMeas(){
	float trig_gears = 0;
	alt_u16 trig_pixel = 0;

	trig_gears = (float)dispdev.trig_level / (float)ver_res_arr[dispdev.ver_res_gears];
	trig_gears = trig_gears + 4;                //转换成正数

	if(trig_gears <0)
		trig_pixel = 0;
	else{
		trig_pixel = dispdev.wave_ystart + dispdev.wave_height - trig_gears * (float)dispdev.wave_height/8.0;
		if(dispdev.ver_zero_mark >= 8)
			trig_pixel = trig_pixel - (dispdev.ver_zero_mark - 8)*dispdev.wave_height/16;
		else
			trig_pixel = trig_pixel + (8 - dispdev.ver_zero_mark)*dispdev.wave_height/16;
	}
	if(trig_pixel > 255)
		trig_pixel = 255;
	return trig_pixel;
