...
void LCD_SetCursor(u16 Xpos, u16 Ypos)
{
    if(lcddev.id==0X9341||lcddev.id==0X5310) {
        ...
        LCD_WR_CMD(lcddev.setxcmd);
        LCD_WR_DATA(Xpos>>8);
        LCD_WR_CMD(lcddev.setxcmd+1);
        LCD_WR_DATA(Xpos&0XFF);
        LCD_WR_CMD(lcddev.setycmd);
        LCD_WR_DATA(Ypos>>8);
        LCD_WR_CMD(lcddev.setycmd+1);
        LCD_WR_DATA(Ypos&0XFF);
    }
}
...
void LCD_Scan_Dir(u8 dir)
{
    ...
    if(lcddev.id==0x9341||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0X1963) { //9341/5310/5510/1963,特殊处理
        ...
        if(lcddev.id==0X5510) {
            ...
            LCD_WR_CMD(lcddev.setxcmd+1);
            LCD_WR_DATA(0);
            LCD_WR_CMD(lcddev.setxcmd+2);
            LCD_WR_DATA((lcddev.width-1)>>8);
            LCD_WR_CMD(lcddev.setxcmd+3);
            LCD_WR_DATA((lcddev.width-1)&0XFF);
            LCD_WR_CMD(lcddev.setycmd);
            LCD_WR_DATA(0);
            LCD_WR_CMD(lcddev.setycmd+1);
            LCD_WR_DATA(0);
            LCD_WR_CMD(lcddev.setycmd+2);
            LCD_WR_DATA((lcddev.height-1)>>8);
            LCD_WR_CMD(lcddev.setycmd+3);
            LCD_WR_DATA((lcddev.height-1)&0XFF);
        } else {
            LCD_WR_CMD(lcddev.setxcmd);
            LCD_WR_DATA(0);
